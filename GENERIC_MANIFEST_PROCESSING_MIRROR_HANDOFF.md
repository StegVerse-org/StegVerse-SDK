...
### Post-admissibility local execution refusal must not be reported as final ALLOW — 2026-09-26

The original standalone SDK source route is general and merged, but the first implementation projected `observation.evaluation.disposition` directly as final `result.disposition`. Actual canonical StegCore may return `evaluation.disposition=ALLOW`, `status=refused`, `executor_invoked=False` when the commit-time present state/coherence denies or its precommit evidence writer fails. The local SDK result now retains the original evaluator outcome separately in `canonical_admissibility_disposition` while using the actual canonical execution outcome and coherent gate receipt to report the effective local `DENY`/`FAIL_CLOSED` instead of a misleading apparent executed ALLOW. Dedicated actual installed SDK source tests simulate observer refusal; protected native StegCore integration provides the separately actual original-runtime test. This correction does **not** issue standing, claim original Interlock/InTr execution authority, alter old remote route semantics, authorize a StegCore distribution, or close ICV's existing product admission task.
