...
## Existing-owner historical hash-DENY correction (2026-09-25)

Existing central PR #2718 merged `ef92bf84877215234a514fc50d443403352b553a`, recording a precise producer-profile manifest-binding DENY only after transport verification; it is not an InTr admission or Master Records receipt. This SDK correction reuses the already installed `derive_execution_request` envelope builder, checks original request and immutable wire hashes, and prepares a distinctly hash-bound correction when the profile's explicit reason code matches a repairable binding defect. Retain the first DENY if the builder emits identical bytes; permit at most one new governed request through the existing Universal InTr public route when a real correction exists. All second-attempt results still pass the existing strict typed validator. An attempted terminal FAIL_CLOSED is never automatically retried. The original Experiment 3 file SHA-256 `e1b05a082ce19d3d254e3cde1dced03019174a94287724959672c9e65510c8f3` and wire hash `ad9b8b8aab2beeea04bff2aac34fd2e7bfa5915133bcaef9209c16de7d9bea68` remain unchanged; Publisher required=true. The source tests do not establish a resident execution or authenticated ledger/Master Records readback.

## Related upcoming testing environments (not product commitments)

See [portable ephemeral governance testing environments](UPCOMING_EPHEMERAL_GOVERNANCE_TEST_ENVIRONMENTS.md) for planned local, event-ephemeral, cross-network, MIR commitment/retrieval and third-party reconstruction scenarios. These scenarios reuse existing SDK and governance owners; they do **not** change the frozen Experiment 3 manifest or its Publisher-required review completion, assert a release roadmap, or claim authentic runtime/MIR verification before original evidence is independently retrieved.
