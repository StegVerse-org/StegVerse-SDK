...
| MIR interface | Existing `StegVerse-Labs/.github:workers/mir_tvc_provider_roundtrip_worker.py` already builds the exact `SUBMIT_EVENT` operation for `https://mirregistry.com/v1/events` using the existing SDK caller identity, generic `RTC-INTERLOCK-INTR-TRANSPORT-008` and TVC non-exportable provider broker. The user confirms the existing MIR submission path is **VERIFIED**; do not reopen submission-path implementation or its completed verification. Historical Run-2 code is not proof that the new Experiment 3 event has already executed or been independently reconstructed. | Reuse the **verified submission path unchanged** and designate actual new-event governance evidence, MIR's retained record identity, independent MIR retrieval, original-evidence acquisition and independent SDK replay/reconstruction as the outstanding integration tests. Confirm interface-specific immutability claims from the original MIR proof. Do **not** add a parallel direct MIR API transport, credential store or governance adapter. |

### Smallest integrative SDK test

Use one source-framework event and its exact immutable manifest. Do not reuse Run-2's historical event body as an Experiment 3 execution sample. Submit it over the existing installed public SDK route; obtain actual native admission and bounded execution dispositions; retain exact organization and Master Records evidence; export that evidence using the already installed exchange; use the already-verified MIR submission path to obtain the **new event's** MIR record/commitment and verify its independent retrieval; and let the originator or a third-party verifier fetch original custody evidence and rerun the existing verifier plus receipt-ID reconstruction. Retain the original source-framework observation separately from MIR's historical commitment and StegVerse's governance evidence.
...
| MIR historical commitment and retrieval | Use the **existing verified** MIR submission path for the next newly governed event; do not rebuild or re-verify the submission mechanism. Independently retrieve the resulting MIR record through its applicable read interface. | New event's MIR-native response/record ID, fresh independent retrieval, record-to-original-evidence exact digest comparison, and immutable/append-only proof **only to the extent supported by the selected MIR interface**. Submission-path verification is already satisfied; new-event retention/readback is a separate test. |
