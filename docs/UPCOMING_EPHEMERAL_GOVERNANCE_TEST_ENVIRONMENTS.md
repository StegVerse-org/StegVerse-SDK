...
| MIR interface | Existing `StegVerse-Labs/.github:workers/mir_tvc_provider_roundtrip_worker.py` already builds the exact `SUBMIT_EVENT` operation for `https://mirregistry.com/v1/events` using the existing SDK caller identity, generic `RTC-INTERLOCK-INTR-TRANSPORT-008` and TVC non-exportable provider broker. This is a historical bounded Run-2 path, **not** proof of an authentic Experiment 3 MIR return. | Reuse the existing authorized MIR operation and designated Interlock/InTr communication seam. Confirm the selected MIR retention/immutable-assertion and retrieval interfaces from authoritative MIR evidence; prove the real operation, authentic MIR-native return, later independent readback and exact commitment comparison. Do **not** add a parallel direct MIR API transport, credential store or governance adapter. |
