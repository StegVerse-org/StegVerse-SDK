...
### DeepWiki 38-page review: expanded evidence (2026-09-25)

Exact prior [review-only workflow 36175208212](https://github.com/StegVerse-org/StegVerse-SDK/actions/runs/36175208212) succeeded with [artifact 10882475515](https://github.com/StegVerse-org/StegVerse-SDK/actions/runs/36175208212/artifacts/10882475515): original 38-page capture unchanged, 613 source-bounded candidates (505 unique URLs); 75 lexical-low-overlap review priorities, 538 overlap-only candidates, 27 unassessable original citations and **zero semantically verified claims**. Remaining 46 empty targets have 7 straightforward newly source-verified proposals, 15 independently source-validated composite subreferences, 18 of 19 malformed-context source probes, and 7 out-of-range simple citation spans requiring explicit bounded correction. These are review proposals, not approved Markdown replacements. See [audit](docs/deepwiki-review/CITATION_AUDIT.md); source-only CI, inherited MIT license and public MCP capture do not clear generated-text reuse, contributor title or official wiki publication.

### DeepWiki capture/inspection source merge and remaining DENY — September 25

[SDK PR #322](https://github.com/StegVerse-org/StegVerse-SDK/pull/322) is merged as `2e3ad70eb5525a49c91631b1aa2907d5c47be034` after 12/12 exact-head PR checks passed. Only review tooling, source-linked audit/inventory, tests, workflow and README/handoff notes entered main; the generated 38-page Markdown was **not** imported into `sdk.stegverse.org`. A directly checked [citation-context discrepancy](docs/deepwiki-review/CITATION_AUDIT.md) shows the generated governance-intake diagram cites README Publisher lines 29-30 as if they established diagram wiring. The remaining semantic audit (613 candidate occurrences/505 unique URLs), 46 link-defect review cases and third-party generated-content reuse clearance remain active under the existing open-source owner; treat unreviewed import as non-ALLOW.


### Correctable historical manifest DENY under the existing SDK builder (2026-09-25)

The exact source-profile `SDK_MANIFEST_BINDING` DENY returned by the existing central manifest ingress can be verified against the rejected original request and corrected by `stegverse.manifest_builder.correct_manifest_binding_deny`. The original frozen Experiment 3 manifest and Publisher requirement remain unchanged. If the corrected envelope has a distinct request digest, the existing SDK universal client can submit it as one separately governed attempt; a repeated unchanged request retains the original actionable DENY without blind replay. The client validates every returned typed source DENY, terminal local FAIL_CLOSED or nonterminal processing ALLOW using the installed source contracts. It never elevates a producer-profile denial to authenticated InTr or retries terminal FAIL_CLOSED. No new runtime, scheduler, token or user-operated device is introduced; authentic native runtime and original organization/Master Records readback remain unverified by source tests.

## DeepWiki frozen-citation priority review and first-party documentation (2026-09-25)

Under existing canonical owner `ECOSYSTEM-OPEN-SOURCE-STRATEGY-001` (COSV `20010010100000`), the retained public 38-page DeepWiki capture stays immutable: SHA-256 `a729d9b0f487f785b0e8f92006f303e8520e3593b6eb6c4c5de43273daa72087`. The review-only `scripts/triage_frozen_deepwiki_citations.py` exports reproducible per-occurrence registers for 75 priority references and 46 unresolved original citation defects. Source excerpts support 27 **narrow literal facts only**, identify two mismatched diagram citations, and leave 46 priority contexts requiring broader review; this is not blanket semantic approval of generated pages. [First-party SDK source guide](docs/deepwiki-review/FIRST_PARTY_SOURCE_GUIDE.md) presents independently worded, precise source-backed developer notes and remains outside the published SDK Pages builder. The 613 original source-bounded candidates and remaining citation proposals stay evidence-only. Generated-text reuse rights, the complete contributor chain, broader semantic accuracy and publication authority remain unverified; no DeepWiki generated Markdown is imported, licensed anew or deployed.

## 2026-09-25 complete per-occurrence 659-target quarantine and three drift corrections

Existing owner: `ECOSYSTEM-OPEN-SOURCE-STRATEGY-001`, COSV `20010010100000` (last observed canonical Registry generation 245 ACTIVE/CHECKED_OUT). Review-only [PR #331](https://github.com/StegVerse-org/StegVerse-SDK/pull/331) adds [`scripts/curate_frozen_deepwiki_review.py`](scripts/curate_frozen_deepwiki_review.py) and tests. It consumes the original SHA-256-pinned 328,247-byte, 38-page DeepWiki capture, earlier raw offset maps, claim/source excerpts, 75-original-priority register and 46 original unresolved refs. Every one of **613 candidate source citations** receives an individual exact-offset, claim-context hash, source-excerpt hash, source-revision and explicit non-ALLOW disposition; this is traceable triage, **not** a claim of complete semantic verification. The earlier 46 broader-priority contexts now receive 46 individual reason-coded `DENY:REVIEWED_INSUFFICIENT_EVIDENCE` decisions; the 27 narrowly verified literals do not approve their surrounding prose, the two demonstrated diagram mismatches remain DENY, and the remaining 538 link candidates stay `DENY:PENDING_CLAIM_SPECIFIC_SEMANTIC_REVIEW`. The **46 original malformed or empty citation references are all omitted as links** in a quarantined derivative (27 simple, 19 malformed). The other 613 unapproved candidate links are likewise downgraded to plain source labels, leaving **zero literal `]()` empty citation targets** in the review-only copy. The unmodified original capture retains its hash. This derivative is **not** copied into the repository or official SDK Pages wiki. Every full generated claim remains unapproved and original indexing-revision parity unestablished.
...
## Source-only DeepWiki atomic citation review — September 25, 2026

The optional source-only [SDK DeepWiki review PR #333](https://github.com/StegVerse-org/StegVerse-SDK/pull/333) extends the previously merged 659-target quarantine audit with an *individual source/claim assessment* of 613 candidate references and the frozen 538/75 review partitions. It does not mistake path/line validity, local test assertions, checked-in evidence flags, or old handoffs for independent runtime proof. A separate [first-party SDK contract guide](docs/deepwiki-review/FIRST_PARTY_VERIFIED_CONTRACTS.md) documents 15 tightly bounded implementation facts, while a [rights decision record](docs/deepwiki-review/GENERATED_DOCS_AND_DEPENDENCY_RIGHTS.md) preserves `DENY` for unlicensed/uncleared public generated-text republication and unverified optional Git-pinned dependency rights. Neither the raw external DeepWiki capture nor a corrected copy is part of the published six-file SDK Pages source list. Full semantic review of unsupported wider generated claims and release/rights authority remain outstanding.

### Historical source evidence is not current-source approval

Source review run 36194717114 rechecked all 27 earlier narrow source facts at SDK revision eec03e0fa3a2ee327eb4e3368b3f8458ac85e582, with explicit predecessor references. Four were independently matched in the newer atomic review; the remaining 23 remain historically verified only. The first-party SDK contract guide and rights matrix preserve this distinction, and the public wiki does not import the generated DeepWiki text.
...
## Linked worker participation histories (source-only experiment; 2026-09-26)
