...
## Optional Publisher; reviewer-facing default (2026-09-24)

Ordinary SDK manifests keep Publisher **optional**. SDK Manifest Builder now treats `for_external_review=True` (CLI `--for-external-review`) as the default to require Publisher for runs that will be returned to independent evaluators. `publisher_required=False` / `--publisher-optional` explicitly overrides; choosing a default does not invoke Publisher. The reusable `stegverse/publisher_review_transfer.py` consumes an exact source-scoped SDK manifest and result plus complete original evidence bytes to prepare the already-installed Publisher `stegverse.publisher.artifact-transfer/v1` packet. It never fabricates an authentic SDK run, origin custody, publication approval or delivery. [Source contract](docs/SDK_EXTERNAL_REVIEW_PUBLISHER_DEFAULT.md), [Publisher existing adapter](https://github.com/GCAT-BCAT-Engine/Publisher/blob/main/publisher/intr_artifact_transfer.py). Current generic SDK-origin transport binding remains a separately evidenced InTr concern; the historical KV-origin Publisher profile must not be impersonated.
