...
STATIC_PACKET_FILES = {
    ROOT / "docs" / "EVALUATION_BOUNDARY_PACKET_README_REPRODUCE.md": "README_REPRODUCE.md",
    ROOT / "docs" / "EVALUATION_BOUNDARY_LICENSE_ACCESS_NOTES.md": "LICENSE_ACCESS_NOTES.md",
}
...
def _tamper_copy(value: dict[str, Any], marker: str) -> dict[str, Any]:
    ...
    copy["__evaluation_boundary_deliberate_tamper__"] = marker
    return copy
...
def build_packet(*, release_receipt_path: Path, run_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    file_manifest = {
        "schema": "stegverse.evaluation-boundary-response-packet-file-manifest.v1",
        "release_set_id": RELEASE_SET_ID,
        "files": files,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build the fail-closed evaluation-boundary response packet")
