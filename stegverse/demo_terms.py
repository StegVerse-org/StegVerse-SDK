...
def accept_demo_terms(*, participant_id: str, signer_name: str, signer_capacity: str, accepted: bool, electronic_signature: str, accepted_at: str | None = None) -> dict[str, Any]:
    participant_id = participant_id.strip(); signer_name = signer_name.strip(); signer_capacity = signer_capacity.strip(); electronic_signature = electronic_signature.strip()
    ...
    receipt: dict[str, Any] = {
        "schema": TERMS_RECEIPT_SCHEMA, "participant_id": participant_id, "signer_name": signer_name, "signer_capacity": signer_capacity,
        "affirmatively_accepted": True, "electronic_signature": electronic_signature, "accepted_at": timestamp,
        "terms": current_demo_terms_descriptor(), "service_relationship_only": True, "software_license_rights_replaced": False,
        "execution_authority_granted": False, "credential_authority_granted": False, "repository_access_granted": False,
    }
...
def verify_demo_terms_acceptance(receipt: Mapping[str, Any]) -> bool:
    ...
    for key in ("participant_id", "signer_name", "signer_capacity", "electronic_signature", "accepted_at"):
        if not isinstance(receipt.get(key), str) or not str(receipt.get(key)).strip(): return False
    if receipt.get("service_relationship_only") is not True: return False
    for key in ("software_license_rights_replaced", "execution_authority_granted", "credential_authority_granted", "repository_access_granted"):
        if receipt.get(key) is not False: return False
    try:
        if receipt.get("terms") != current_demo_terms_descriptor(): return False
    except RuntimeError:
        return False
