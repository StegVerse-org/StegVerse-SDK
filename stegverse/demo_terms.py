...
def current_demo_terms_descriptor() -> dict[str, Any]:
    ...
    return {
        "terms_of_service": {
            ...
            "sha256": _sha256_bytes(tos.read_bytes()),
            "path": "legal/demo/DEMO_TERMS_OF_SERVICE.md",
        },
        "terms_of_use": {
            "version": TERMS_OF_USE_VERSION,
            "sha256": _sha256_bytes(tou.read_bytes()),
            "path": "legal/demo/DEMO_TERMS_OF_USE.md",
        },
    }


def accept_demo_terms(
    *,
    participant_id: str,
    signer_name: str,
    signer_capacity: str,
...
def verify_demo_terms_acceptance(receipt: Mapping[str, Any]) -> bool:
    if receipt.get("schema") != TERMS_RECEIPT_SCHEMA:
        return False
    if receipt.get("affirmatively_accepted") is not True:
        return False
