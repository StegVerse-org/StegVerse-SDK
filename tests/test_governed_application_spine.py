...
from stegverse.governed_application_spine import CANONICAL_STEGGATE_RUNTIME, canonical_hash, validate_governed_application_spine
...
def _record(*, execute=False):
    return {
        ...
        "run_id": "run-001",
        "source": {"origin_class": "SDK_INPUT", "source_hash": HASH_A, "artifact_refs": ["attachment:example"]},
        "candidate": {"candidate_hash": HASH_B, "authorizing": False, "model_output_authority": "NONE"},
        "standing": {"state": "ALLOW" if execute else "PENDING", "receipt_hash": HASH_C if execute else None, "execution_authorized": False, "standing_current": True if execute else None, "validity_window": None},
        "admissibility": {"runtime_identity": CANONICAL_STEGGATE_RUNTIME, "request_hash": HASH_D if execute else None, "state": "ALLOW" if execute else "PENDING", "commit_time_validity": "CURRENT" if execute else "PENDING", "commit_coherence": "ALLOW" if execute else "PENDING"},
        "execution": {"performed": execute, "executor_ref": "executor:test" if execute else None, "result_hash": HASH_E if execute else None},
        "continuity": {"return_ingested": execute, "receipt_chain_head": HASH_E if execute else None, "reconstruction_state": "PASS" if execute else "PENDING", "discovery_ref": None, "custody_ref": None},
        "authority": {"sdk_authority": "NONE", "spe_execution_authority": "NONE", "model_output_authority": "NONE", "custody_authority": "NONE"},
    }
