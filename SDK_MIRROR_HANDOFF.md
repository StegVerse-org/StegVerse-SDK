...
Canonical owner: `StegVerse-Labs/.github:data/canonical-task-records/SDK-MICRO-NODE-COMMIT-TIME-ADMISSIBILITY-001.json`; COSV `71000000111111`; canonical .github handoff `docs/SDK_MICRO_NODE_COMMIT_TIME_ADMISSIBILITY_MIRROR_HANDOFF.md`. Branch `feat/sdk-micro-node-admissibility-contract-001` introduces only a pure SDK-local validator for the existing public three-worker group-manifest/result, with positive and adversarial tests. It binds the exact manifest hash, recomputed SDK group result/partition identity, individual attribution and observation status, recorded CTA disposition (including ESCALATE/REFUSE), current-standing/warrant claims and claimed exact predecessor reconstruction. Even all-passing caller-supplied fixtures report NON_AUTHORIZING_LOCAL_REVIEW_ONLY; no new execution route, scheduler, custody, credential or authority plane is created. Live independent worker origin/custody, actual InTr determination and Master Records reconstruction remain UNKNOWN pending authentic receipts from existing owners.
